...
function getDashboardStats(PDO $pdo) {
    ...
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM eclusa_movimentacoes");
    $total = $stmt->fetch()['total'];

    // Movimentações hoje
    $stmt = $pdo->prepare("SELECT COUNT(*) as hoje FROM eclusa_movimentacoes WHERE DATE(data_movimentacao) = CURDATE()");
    $stmt->execute();
    $hoje = $stmt->fetch()['hoje'];

    // Movimentações entrada hoje
    $stmt = $pdo->prepare("SELECT COUNT(*) as entradas FROM eclusa_movimentacoes WHERE DATE(data_movimentacao) = CURDATE() AND hora_entrada IS NOT NULL");
    $stmt->execute();
    $entradas = $stmt->fetch()['entradas'];

    // Movimentações saída hoje
    $stmt = $pdo->prepare("SELECT COUNT(*) as saidas FROM eclusa_movimentacoes WHERE DATE(data_movimentacao) = CURDATE() AND hora_saida IS NOT NULL");
    $stmt->execute();
    $saidas = $stmt->fetch()['saidas'];

    // Movimentações esta semana
    $stmt = $pdo->prepare("SELECT COUNT(*) as semana FROM eclusa_movimentacoes WHERE YEARWEEK(data_movimentacao) = YEARWEEK(CURDATE())");
    $stmt->execute();
    $semana = $stmt->fetch()['semana'];

    // Movimentações este mês
    $stmt = $pdo->prepare("SELECT COUNT(*) as mes FROM eclusa_movimentacoes WHERE YEAR(data_movimentacao) = YEAR(CURDATE()) AND MONTH(data_movimentacao) = MONTH(CURDATE())");
    $stmt->execute();
    $mes = $stmt->fetch()['mes'];
...
function getMovimentacoesPorData(PDO $pdo, $periodo = '30') {
    $intervalos = [
        '7' => '7 DAY',
        '30' => '30 DAY',
        '90' => '90 DAY',
    ...
    $sql = "
        ...
        WHERE data_movimentacao >= DATE_SUB(CURDATE(), INTERVAL $interval)
...
function getMovimentacoesPorTipo(PDO $pdo) {
    $sql = "
        SELECT
            CASE
                WHEN hora_entrada IS NOT NULL AND hora_saida IS NOT NULL THEN 'entrada_saida'
                WHEN hora_entrada IS NOT NULL THEN 'entrada'
                WHEN hora_saida IS NOT NULL THEN 'saida'
                ELSE 'indefinido'
            END as tipo,
            COUNT(*) as quantidade
        FROM eclusa_movimentacoes
...
function getTopVeiculos(PDO $pdo, $limit = 10) {
    $sql = "
        SELECT
            COALESCE(v.nome, 'Não identificado') as veiculo,
            v.placa,
            COUNT(m.id) as total_movimentacoes,
            COUNT(DISTINCT DATE(m.data_movimentacao)) as dias_ativos,
            MAX(DATE(m.data_movimentacao)) as ultima_movimentacao
...
function getTopEmpresas(PDO $pdo, $limit = 10) {
    $sql = "
        SELECT
            COALESCE(e.nome, 'Não identificada') as empresa,
            COUNT(m.id) as total_movimentacoes,
            COUNT(DISTINCT DATE(m.data_movimentacao)) as dias_ativos,
            MAX(DATE(m.data_movimentacao)) as ultima_movimentacao
        FROM eclusa_movimentacoes m
...
function getMovimentacoesPorHora(PDO $pdo) {
    $sql = "
        SELECT
            HOUR(COALESCE(hora_chegada, hora_entrada, hora_saida)) as hora,
            COUNT(*) as quantidade
        FROM eclusa_movimentacoes
        WHERE hora_chegada IS NOT NULL OR hora_entrada IS NOT NULL OR hora_saida IS NOT NULL
        GROUP BY hora
        ORDER BY hora ASC
    ";
...
function getMovimentacoesPorDia(PDO $pdo) {
    $sql = "
        SELECT
            DAYOFWEEK(data_movimentacao) as dia_semana_num,
            CASE DAYOFWEEK(data_movimentacao)
                WHEN 1 THEN 'Domingo'
                WHEN 2 THEN 'Segunda'
                WHEN 3 THEN 'Terça'
                WHEN 4 THEN 'Quarta'
...
function getMovimentacoesPorMes(PDO $pdo) {
    $sql = "
        SELECT
            ...
            COUNT(*) as quantidade
        FROM eclusa_movimentacoes
        WHERE data_movimentacao >= DATE_SUB(CURDATE(), INTERVAL 12 MONTH)
...
// Funções de detalhes para o dashboard
function getMovimentacoesDetalhadasPorData(PDO $pdo, $data) {
    $sql = "
        SELECT
            m.id,
            m.data_movimentacao,
        ...
        ORDER BY m.data_movimentacao, m.hora_chegada, m.hora_entrada, m.hora_saida
    ";
...
function getMovimentacoesDetalhadasPorTipo(PDO $pdo, $tipo) {
    $sql = "
        SELECT
            m.id,
            m.data_movimentacao,
        ...
        WHERE CASE
                ...
                WHEN m.hora_entrada IS NOT NULL THEN 'entrada'
                WHEN m.hora_saida IS NOT NULL THEN 'saida'
                ELSE 'indefinido'
            END = ?
...
function getMovimentacoesDetalhadasPorVeiculo(PDO $pdo, $veiculo) {
    $sql = "
        SELECT
            m.id,
            m.data_movimentacao,
...
function getMovimentacoesDetalhadasPorEmpresa(PDO $pdo, $empresa) {
    $sql = "
        SELECT
            m.id,
            m.data_movimentacao,
...
function getMovimentacoesDetalhadasPorHora(PDO $pdo, $hora) {
    $sql = "
        SELECT
            m.id,
            m.data_movimentacao,
        ...
        LEFT JOIN eclusa_empresas e ON e.id = m.empresa_id
        LEFT JOIN eclusa_motoristas mo ON mo.id = m.motorista_id
        WHERE HOUR(COALESCE(m.hora_chegada, m.hora_entrada, m.hora_saida)) = ?
        ORDER BY m.data_movimentacao DESC
    ";
...
function getMovimentacoesDetalhadasPorDiaSemana(PDO $pdo, $dia) {
    $sql = "
        SELECT
            m.id,
            m.data_movimentacao,
        ...
        LEFT JOIN eclusa_empresas e ON e.id = m.empresa_id
        LEFT JOIN eclusa_motoristas mo ON mo.id = m.motorista_id
        WHERE DAYNAME(m.data_movimentacao) = ?
        ORDER BY m.data_movimentacao DESC
    ";
...
function getMovimentacoesDetalhadasPorMes(PDO $pdo, $mes) {
    $sql = "
        SELECT
            m.id,
            m.data_movimentacao,
        ...
        LEFT JOIN eclusa_empresas e ON e.id = m.empresa_id
        LEFT JOIN eclusa_motoristas mo ON mo.id = m.motorista_id
        WHERE DATE_FORMAT(m.data_movimentacao, '%Y-%m') = ?
        ORDER BY m.data_movimentacao DESC
    ";
...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['db_action'])) {
    ...
    try {
        ...
        if ($action === 'get_kpi_detalhes') {
            $kpi = in_str('kpi');
            if (!$kpi) {
                eclusa_error('KPI não informado.');
            }
            ...
            switch ($kpi) {
                case 'total':
                    ...
                    $sql = "
                        ...
                        FROM eclusa_movimentacoes
                        WHERE data_movimentacao >= DATE_SUB(CURDATE(), INTERVAL 12 MONTH)
                        GROUP BY DATE_FORMAT(data_movimentacao, '%m/%Y')
                        ORDER BY MIN(data_movimentacao) ASC
                    ";
                    $stmt = $pdo->prepare($sql);
                    $stmt->execute();
                    $data['mensal'] = $stmt->fetchAll();
                    break;
                ...
                case 'hoje':
                    ...
                    $sql = "
                        SELECT
                            COUNT(*) as total,
                            CASE
                                WHEN hora_entrada IS NOT NULL AND hora_saida IS NULL THEN 'entrada'
                                WHEN hora_saida IS NOT NULL AND hora_entrada IS NULL THEN 'saida'
                                WHEN hora_entrada IS NOT NULL AND hora_saida IS NOT NULL THEN 'entrada_saida'
                                ELSE 'indefinido'
                            END as tipo,
                            COUNT(*) as quantidade
                ...
                case 'entradas':
                    ...
                    $sql = "
                        ...
                        LEFT JOIN eclusa_empresas e ON e.id = m.empresa_id
                        LEFT JOIN eclusa_motoristas mot ON mot.id = m.motorista_id
                        WHERE m.data_movimentacao = CURDATE() AND m.hora_entrada IS NOT NULL
                        ORDER BY m.hora_entrada ASC
                    ";
                    $stmt = $pdo->prepare($sql);
                    $stmt->execute();
                    $data['entradas'] = $stmt->fetchAll();
                    break;
                ...
                case 'saidas':
                    ...
                    $sql = "
                        ...
                        LEFT JOIN eclusa_empresas e ON e.id = m.empresa_id
                        LEFT JOIN eclusa_motoristas mot ON mot.id = m.motorista_id
                        WHERE m.data_movimentacao = CURDATE() AND m.hora_saida IS NOT NULL
                        ORDER BY m.hora_saida ASC
                    ";
                    $stmt = $pdo->prepare($sql);
                    $stmt->execute();
                    $data['saidas'] = $stmt->fetchAll();
                    break;

                case 'veiculos':
                    $titulo = 'Top Veículos';
                    $data['top_veiculos'] = get_top_veiculos($pdo);
                    break;

                case 'empresas':
                    $titulo = 'Top Empresas';
                    $data['top_empresas'] = get_top_empresas($pdo);
                    break;
            }

            eclusa_ok(['data' => $data, 'titulo' => $titulo]);
        }
        ...
        if ($action === 'get_chart_data') {
            ...
            if ($chartType === 'data') {
                eclusa_ok(['data' => getMovimentacoesPorData($pdo, $periodo)]);
            } elseif ($chartType === 'tipo') {
                eclusa_ok(['data' => getMovimentacoesPorTipo($pdo)]);
            } elseif ($chartType === 'hora') {
                eclusa_ok(['data' => getMovimentacoesPorHora($pdo)]);
            } elseif ($chartType === 'dia') {
                eclusa_ok(['data' => getMovimentacoesPorDia($pdo)]);
            } elseif ($chartType === 'mes') {
                eclusa_ok(['data' => getMovimentacoesPorMes($pdo)]);
            } else {
                eclusa_error('Tipo de gráfico não reconhecido.');
            }
        }

        if ($action === 'get_movimentacoes_detalhadas') {
            $data = in_str('data');
            if (!$data) {
                eclusa_error('Data não informada.');
            }
            eclusa_ok(['data' => getMovimentacoesDetalhadasPorData($pdo, $data)]);
        }

        if ($action === 'get_movimentacoes_por_tipo') {
            $tipo = in_str('tipo');
            if (!$tipo) {
                eclusa_error('Tipo não informado.');
            }
            eclusa_ok(['data' => getMovimentacoesDetalhadasPorTipo($pdo, $tipo)]);
        }

        if ($action === 'get_movimentacoes_por_veiculo') {
            $veiculo = in_str('veiculo');
            if (!$veiculo) {
                eclusa_error('Veículo não informado.');
            }
            eclusa_ok(['data' => getMovimentacoesDetalhadasPorVeiculo($pdo, $veiculo)]);
        }

        if ($action === 'get_movimentacoes_por_empresa') {
            $empresa = in_str('empresa');
            if (!$empresa) {
                eclusa_error('Empresa não informada.');
            }
            eclusa_ok(['data' => getMovimentacoesDetalhadasPorEmpresa($pdo, $empresa)]);
        }

        if ($action === 'get_movimentacoes_por_hora') {
            $hora = in_str('hora');
            if (!$hora) {
                eclusa_error('Hora não informada.');
            }
            eclusa_ok(['data' => getMovimentacoesDetalhadasPorHora($pdo, $hora)]);
        }

        if ($action === 'get_movimentacoes_por_dia_semana') {
            $dia = in_str('dia');
            if (!$dia) {
                eclusa_error('Dia da semana não informado.');
            }
            eclusa_ok(['data' => getMovimentacoesDetalhadasPorDiaSemana($pdo, $dia)]);
        }

        if ($action === 'get_movimentacoes_por_mes') {
            $mes = in_str('mes');
            if (!$mes) {
                eclusa_error('Mês não informado.');
            }
            eclusa_ok(['data' => getMovimentacoesDetalhadasPorMes($pdo, $mes)]);
        }
