(function(){
    ...
    console.log('✅ Script censura_rouparia_numeros.js v2.0 carregado - bootstrapRouparia ativado');
    ...
    window.cadastrarKit = async (e) => {
        ...
        try {
            const res = await fetch('paginas/censura_rouparia_numeros.php', { method: 'POST', body: fd });
    ...
    window.saveOff = async e => {
        e.preventDefault();
        const res = await fetch('paginas/censura_rouparia_numeros.php', { method: 'POST', body: new FormData(e.target) });
        const json = await res.json();
        if(json.success) {
            ...
            window.reloadContent('paginas/censura_rouparia_numeros.php?' + qs);
    ...
    window.salvarCadastro = async (e) => {
        ...
        try {
            ...
            if(json.success) {
                ...
                if(typeof loadPage === 'function') {
                    loadPage('paginas/censura_rouparia_numeros.php');
                } else {
                    window.location.href = 'paginas/censura_rouparia_numeros.php';
                }
            } else {
                alert('Erro: ' + json.error);
                btn.disabled = false;
    ...
    window.parent.editarKit = async (id) => {
        const res = await fetch('paginas/censura_rouparia_numeros.php?acao=get_kit&id=' + id);
    ...
    window.buscarHistoricoKit = () => {
        ...
        $.post('paginas/censura_rouparia_numeros.php', {db_action: 'buscar_historico_kit', kit_numero: kit}, function(data) {
    ...
    window.buscarItensPerdidosRoupaCivil = () => {
        ...
        $.post('paginas/censura_rouparia_numeros.php', {db_action: 'buscar_itens_perdidos_roupa_civil'}, function(data) {
...
Esses registros serão exibidos em destaque vermelho para correção.`;
    ...
    window.carregarListaRoupaCivil = () => {
        $.post('paginas/censura_rouparia_numeros.php', {db_action: 'listar_roupa_civil'}, function(data) {
    ...
    window.editarRoupaCivil = async (id) => {
        try {
            const res = await fetch('paginas/censura_rouparia_numeros.php', {
                method: 'POST',
...
window.etiquetasRoupaCivilData = [];
