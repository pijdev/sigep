...
$pdo = null;
try {
    $config = require 'C:\Sites\sigep\conf\db.php';
    ...
    try {
        if ($_POST['action'] === 'fetch_cela_internos') {
            ...
            $data_limite = $mostrar_todos
                ? $ultima_importacao
                : date('Y-m-d H:i:s', strtotime('-48 hours')); // 2 dias no offcanvas
            ...
            if ($bloco !== '') {
                ...
                if (in_array($galeria, ['SA', 'SB', 'SC', 'SD', 'SE', 'ST'])) {
                    ...
                    $query = "
                        SELECT ipen, nome, nome_social, status, situacao, regalia, kit, tipo_residencia,
                               CASE
                                   WHEN (galeria = 'E' AND res IN ('9', '10')) OR
                                        (galeria = 'SE' AND res = '1')
                                   THEN 1
                                   ELSE 0
                               END as eh_lgbt
                        FROM internos
            ...
            $internos_raw = $stmt->fetchAll();

            // Aplicar regra crítica: Nome preferencial para LGBT
            $internos = [];
            foreach ($internos_raw as $interno) {
                $eh_lgbt = (bool)$interno['eh_lgbt'];
                $nome_info = getNomePreferencial($interno['nome'], $interno['nome_social'], $eh_lgbt);

                $internos[] = array_merge($interno, [
                    'nome_exibicao' => $nome_info['nome_exibicao'],
                    'nome_registro' => $nome_info['nome_registro'],
                    'usou_social' => $nome_info['usou_social'],
                    'eh_lgbt' => $eh_lgbt
                ]);
            }
            ...
            foreach ($internos as $interno) {
                ...
                try {
                    $hist_query = "
                        ...
                        WHERE ipen = :ipen AND data_alteracao >= :data_limite AND (
                    ...
                    $hist_stmt->execute([':ipen' => $interno['ipen'], ':data_limite' => $data_limite]);
            ...
            // Histórico completo de movimentações da cela
            $movimentacoes = [];
            try {
                $sql_movimentacoes = "
                    SELECT
                        i.ipen,
                        i.nome,
                        CONCAT(i.bloco, i.galeria, '-', i.res) AS local_atual,
                        CASE
                            WHEN hd.operacao = 'INATIVADO' THEN 'SAIU DO SISTEMA'
                            WHEN hd.valor_novo = 'C' AND hd.campo = 'galeria' THEN 'ENTROU NA GALERIA C'
                            WHEN hd.valor_antigo = 'C' AND hd.campo = 'galeria' THEN 'SAIU DA GALERIA C'
                            WHEN hd.valor_novo = :cela_num AND hd.campo = 'res' THEN 'ENTROU NA CELA " . $galeria . ($bloco ?: "") . "-" . $cela . "'
                            WHEN hd.valor_antigo = :cela_num AND hd.campo = 'res' THEN 'SAIU DA CELA " . $galeria . ($bloco ?: "") . "-" . $cela . "'
                            ELSE 'OUTRA MOVIMENTAÇÃO'
                        END AS tipo_movimentacao,
                        CASE
                            WHEN hd.operacao = 'INATIVADO' THEN 'INATIVO'
                            WHEN hd.valor_novo = 'C' AND hd.campo = 'galeria' THEN
                                CONCAT('DE ', COALESCE(hd.valor_antigo, 'GALERIA NÃO INFORMADA'))
                            WHEN hd.valor_antigo = 'C' AND hd.campo = 'galeria' THEN
                                CONCAT('PARA ', COALESCE(hd.valor_novo, 'GALERIA NÃO INFORMADA'))
                            WHEN hd.valor_novo = :cela_num AND hd.campo = 'res' THEN
                                CONCAT('DE ', COALESCE(hd.valor_antigo, 'N/A'))
                            WHEN hd.valor_antigo = :cela_num AND hd.campo = 'res' THEN
                                CONCAT('PARA ', COALESCE(hd.valor_novo, 'N/A'))
                            ELSE CONCAT('DE ', COALESCE(hd.valor_antigo, 'N/A'), ' PARA ', COALESCE(hd.valor_novo, 'N/A'))
                        END AS origem_destino_completo,
                        hd.data_alteracao,
                        hd.campo,
                        hd.operacao
                    FROM internos_historico_detalhado hd
                    INNER JOIN internos i ON hd.ipen = i.ipen
                    WHERE (
                        (hd.valor_novo = 'C' AND hd.campo = 'galeria') OR
                        (hd.valor_antigo = 'C' AND hd.campo = 'galeria') OR
                        (hd.valor_novo = :cela_num AND hd.campo = 'res') OR
                        (hd.valor_antigo = :cela_num AND hd.campo = 'res') OR
                        hd.operacao = 'INATIVADO'
                    )
                    AND hd.data_alteracao >= :data_limite
                    ORDER BY hd.data_alteracao DESC
                    LIMIT 50
                ";
                $stmt_mov = $pdo->prepare($sql_movimentacoes);
                $stmt_mov->execute([
                    ':cela_num' => $cela,
        ...
        if ($_POST['action'] === 'search_interno') {
            ...
            $query = "
                SELECT DISTINCT ipen, nome, nome_social, galeria, bloco, ala, res, tipo_residencia,
                       CASE
                           WHEN (galeria = 'E' AND res IN ('9', '10')) OR
                                (galeria = 'SE' AND res = '1')
                           THEN 1
            ...
            // Aplicar regra crítica: Nome preferencial para LGBT
            $resultados = [];
            foreach ($resultados_raw as $interno) {
                $eh_lgbt = (bool)($interno['eh_lgbt'] ?? 0);
                $nome_info = getNomePreferencial($interno['nome'], $interno['nome_social'], $eh_lgbt);

                $resultados[] = array_merge($interno, [
                    'nome_exibicao' => $nome_info['nome_exibicao'],
                    'nome_registro' => $nome_info['nome_registro'],
                    'usou_social' => $nome_info['usou_social'],
                    'eh_lgbt' => $eh_lgbt
        ...
        if ($_POST['action'] === 'get_eletronicos_detalhes') {
            ...
            try {
                ...
                if (!empty($internos_cela)) {
                    ...
                    $query_md = "
                        SELECT i.ipen, i.nome, i.nome_social, md.data_inicio, md.data_fim, md.motivo,
        ...
        } elseif ($_POST['action'] === 'validar_movimentacao') {
            // VALIDAR SE MOVIMENTAÇÃO É PERMITIDA (REGRA CRÍTICA: CONVÍVIO vs SEGURO)
            $ipens = $_POST['ipens'] ?? [];
            $galeria_destino = $_POST['galeria_destino'] ?? '';
            $bloco_destino = $_POST['bloco_destino'] ?? '';
            $cela_destino = $_POST['cela_destino'] ?? '';

            if (empty($ipens) || empty($galeria_destino) || empty($cela_destino)) {
                echo json_encode(['success' => false, 'error' => 'Parâmetros inválidos']);
                exit;
            }
            ...
            try {
                $validacao = validarConvivioPermitido($pdo, $ipens, $galeria_destino, $bloco_destino, $cela_destino);
                ...
                if ($validacao['permitido']) {
                    echo json_encode([
                        ...
                        'mensagem' => '✅ Movimentação permitida: ' . $validacao['motivo']
                    ]);
                } else {
                    echo json_encode([
                        'success' => true,
                        'permitido' => false,
                        'motivo' => $validacao['motivo'],
                        'risco' => $validacao['risco'] ?? 'ALTO',
                        'destino_tem' => $validacao['destino_tem'] ?? '',
                        'mover_tem' => $validacao['mover_tem'] ?? '',
                        'mensagem' => '🚨 BLOQUEADO: ' . $validacao['motivo']
                    ]);
                }
                exit;
            } catch (Exception $e) {
                echo json_encode(['success' => false, 'error' => 'Erro na validação: ' . $e->getMessage()]);
                exit;
            }
        } else {
...
// FUNÇÃO PARA OBTER NOME PREFERENCIAL DO INTERNO (REGRA LGBT)
function getNomePreferencial($nome_registro, $nome_social, $eh_lgbt = false)
{
    // REGRA CRÍTICA: Internos LGBT SEMPRE usam nome social se cadastrado
    if ($eh_lgbt && !empty(trim($nome_social))) {
        return [
            'nome_exibicao' => trim($nome_social),
            'nome_registro' => trim($nome_registro),
            'usou_social' => true
        ];
    }

    // Para demais internos, usa nome social se disponível
    if (!empty(trim($nome_social))) {
        return [
            'nome_exibicao' => trim($nome_social),
            'nome_registro' => trim($nome_registro),
            'usou_social' => true
    ...
    return [
        'nome_exibicao' => trim($nome_registro),
        'nome_registro' => trim($nome_registro),
        'usou_social' => false
    ];
}

// FUNÇÃO PARA VALIDAR CONVÍVIO ENTRE INTERNOS
function validarConvivioPermitido($pdo, $ipens_destino, $galeria_destino, $bloco_destino, $cela_destino)
{
    // Verificar se é galeria especial (triagem, isolamento, enfermaria)
    $galerias_especiais = ['F', 'G', 'T']; // F=Enfermaria, G=Isolamento, T=Triagem
    $eh_galeria_especial = in_array($galeria_destino, $galerias_especiais);

    // Verificar se é galeria semi-aberto
    $galerias_semi_aberto = ['SA', 'SB', 'SC', 'SD', 'SE', 'ST'];
    $eh_galeria_semi_aberto = in_array($galeria_destino, $galerias_semi_aberto);

    if ($eh_galeria_especial || $eh_galeria_semi_aberto) {
        return validarGaleriaEspecial($pdo, $ipens_destino, $galeria_destino, $bloco_destino, $cela_destino);
    }

    // Lógica original para galerias normais
    return validarGaleriaNormal($pdo, $ipens_destino, $galeria_destino, $bloco_destino, $cela_destino);
}

// FUNÇÃO PARA VALIDAR GALERIAS ESPECIAIS (Triagem, Isolamento, Enfermaria, Semi-Aberto)
function validarGaleriaEspecial($pdo, $ipens_destino, $galeria_destino, $bloco_destino, $cela_destino)
{
    // Para galerias semi-aberto, converter para formato do banco
    $galeria_banco = $galeria_destino;
    $bloco_banco = $bloco_destino;

    // Se for galeria semi-aberto (SA, SB, SC, SD, SE, ST)
    if (in_array($galeria_destino, ['SA', 'SB', 'SC', 'SD', 'SE', 'ST'])) {
        $galeria_banco = 'S'; // No banco é sempre 'S'
        $bloco_banco = substr($galeria_destino, 1); // Extrai A de SA, B de SB, etc.
    }

    // Buscar tipos dos internos que já estão na cela de destino
    $query_destino = "
        SELECT ipen, nome, tipo_residencia
        FROM internos
        WHERE status = 'A' AND galeria = :gal AND bloco = :bloco AND res = :cela
    ";
    $stmt_destino = $pdo->prepare($query_destino);
    $stmt_destino->execute([':gal' => $galeria_banco, ':bloco' => $bloco_banco, ':cela' => $cela_destino]);
    $internos_destino = $stmt_destino->fetchAll();

    // Buscar tipos dos internos que serão movidos
    $placeholders = implode(',', array_fill(0, count($ipens_destino), '?'));
    $query_mover = "
        SELECT ipen, nome, tipo_residencia
        FROM internos
        WHERE ipen IN ($placeholders) AND status = 'A'
    ";
    $stmt_mover = $pdo->prepare($query_mover);
    $stmt_mover->execute($ipens_destino);
    $internos_mover = $stmt_mover->fetchAll();
    ...
    foreach ($internos_destino as $interno) {
        $tipos_destino[] = $interno['tipo_residencia'];
        if ($interno['tipo_residencia'] === 'Cela') $convivio_destino++;
        elseif ($interno['tipo_residencia'] === 'Seguro') $seguro_destino++;
        else $outros_destino++;
    }

    // Analisar tipos dos internos que serão movidos
    $tipos_mover = [];
    $convivio_mover = 0;
    $seguro_mover = 0;
    $outros_mover = 0;

    foreach ($internos_mover as $interno) {
        $tipos_mover[] = $interno['tipo_residencia'];
        if ($interno['tipo_residencia'] === 'Cela') $convivio_mover++;
        elseif ($interno['tipo_residencia'] === 'Seguro') $seguro_mover++;
        else $outros_mover++;
    }

    // Combinar tipos após movimentação
    $total_convivio = $convivio_destino + $convivio_mover;
    $total_seguro = $seguro_destino + $seguro_mover;
    $total_outros = $outros_destino + $outros_mover;

    // Montar resposta detalhada
    $resposta = [
        'permitido' => true,
        'motivo' => 'Galeria especial - análise detalhada',
        'galeria_tipo' => getNomeGaleriaEspecial($galeria_destino),
        'composicao_atual' => [
            'convivio' => $convivio_destino,
            'seguro' => $seguro_destino,
            'outros' => $outros_destino,
            'total' => count($internos_destino)
        ],
        'internos_mover' => [
            'convivio' => $convivio_mover,
            'seguro' => $seguro_mover,
            'outros' => $outros_mover,
            'total' => count($internos_mover)
        ],
        'composicao_final' => [
            'convivio' => $total_convivio,
            'seguro' => $total_seguro,
            'outros' => $total_outros,
            'total' => count($internos_destino) + count($internos_mover)
        ],
        'alerta_mistura' => false,
        'nivel_risco' => 'BAIXO'
    ];

    // VERIFICAR SE TERÁ MISTURA PERIGOSA APÓS MOVIMENTAÇÃO
    if ($total_convivio > 0 && $total_seguro > 0) {
        $resposta['permitido'] = false;
        $resposta['alerta_mistura'] = true;
        $resposta['motivo'] = "🚨 ALERTA CRÍTICO: Esta movimentação criará uma mistura perigosa em {$resposta['galeria_tipo']}!";
        $resposta['nivel_risco'] = 'CRÍTICO';
        $resposta['detalhe_alerta'] = "Internos de Convívio e Seguro não podem compartilhar o mesmo espaço, mesmo em galerias especiais.";
    }

    // Adicionar informações dos internos para detalhamento
    $resposta['internos_destino_detalhes'] = $internos_destino;
    $resposta['internos_mover_detalhes'] = $internos_mover;

    return $resposta;
}

// FUNÇÃO PARA VALIDAR GALERIAS NORMAIS (lógica original)
function validarGaleriaNormal($pdo, $ipens_destino, $galeria_destino, $bloco_destino, $cela_destino)
{
    // Buscar tipo_residencia dos internos que já estão na cela de destino
    $query_destino = "
        SELECT DISTINCT tipo_residencia
        FROM internos
        WHERE status = 'A' AND galeria = :gal AND bloco = :bloco AND res = :cela
    ";
    $stmt_destino = $pdo->prepare($query_destino);
    $stmt_destino->execute([':gal' => $galeria_destino, ':bloco' => $bloco_destino, ':cela' => $cela_destino]);
    $tipos_destino = $stmt_destino->fetchAll(PDO::FETCH_COLUMN, 0);

    // Se não tem ninguém na cela de destino, permite qualquer tipo
    if (empty($tipos_destino)) {
        return ['permitido' => true, 'motivo' => 'Cela vazia'];
    }

    // Buscar tipo_residencia dos internos que serão movidos
    if (empty($ipens_destino)) {
        return ['permitido' => false, 'motivo' => 'Nenhum interno informado'];
    }
    ...
    $stmt_mover = $pdo->prepare($query_mover);
    $stmt_mover->execute($ipens_destino);
    $tipos_mover = $stmt_mover->fetchAll(PDO::FETCH_COLUMN, 0);

    // Verificar regra crítica: Convívio vs Seguro
    $tem_convivio_destino = in_array('Cela', $tipos_destino);
    $tem_seguro_destino = in_array('Seguro', $tipos_destino);
    $tem_convivio_mover = in_array('Cela', $tipos_mover);
    $tem_seguro_mover = in_array('Seguro', $tipos_mover);

    // REGRA CRÍTICA: Não misturar Convívio com Seguro
    if (($tem_convivio_destino && $tem_seguro_mover) || ($tem_seguro_destino && $tem_convivio_mover)) {
        $motivo = '🚨 VIOLAÇÃO DE SEGURANÇA: Não é permitido misturar internos de "Convívio" (facções) com "Seguro" (protegidos)!';
        return [
            'permitido' => false,
            'motivo' => $motivo,
            'risco' => 'ALTISSIMO',
            'destino_tem' => $tem_convivio_destino ? 'Convívio' : 'Seguro',
            'mover_tem' => $tem_convivio_mover ? 'Convívio' : 'Seguro'
        ];
    }

    return ['permitido' => true, 'motivo' => 'Convívio compatível'];
}

// FUNÇÃO PARA OBTER NOME DA GALERIA ESPECIAL
function getNomeGaleriaEspecial($galeria)
{
    $nomes = [
        'F' => 'Enfermaria',
        'G' => 'Isolamento',
        'T' => 'Triagem/Castigo',
        // Galerias Semi-Aberto
        'SA' => 'Semi-Aberto A',
        'SB' => 'Semi-Aberto B',
        'SC' => 'Semi-Aberto C',
        'SD' => 'Semi-Aberto D',
    ...
    return $nomes[$galeria] ?? "Galeria $galeria";
}

// FUNÇÃO PARA OBTER CORES E ÍCONES DO TIPO DE RESIDÊNCIA
function getTipoResidenciaInfo($tipo_residencia)
{
    $tipos = [
        'Cela' => [
            'cor' => '#dc3545',
            ...
            'icone' => 'fas fa-users',
            'classe' => 'badge-cela',
            'nome_exibicao' => 'Convívio',
            'descricao' => 'Internos com convívio entre facções'
        ],
        'Seguro' => [
            'cor' => '#28a745',
            'cor_texto' => '#ffffff',
            ...
            'classe' => 'badge-seguro',
            'nome_exibicao' => 'Seguro',
            'descricao' => 'Internos sem convívio com facções (protegidos)'
        ],
        'Cela especial' => [
            'cor' => '#6f42c1',
            'cor_texto' => '#ffffff',
            'icone' => 'fas fa-exclamation-triangle',
            'classe' => 'badge-cela-especial'
        ],
        'Cela para PNE' => [
            'cor' => '#17a2b8',
            'cor_texto' => '#ffffff',
            'icone' => 'fas fa-wheelchair',
            'classe' => 'badge-cela-pne'
        ],
        'Enfermaria' => [
            'cor' => '#fd7e14',
            'cor_texto' => '#212529',
            'icone' => 'fas fa-medkit',
            'classe' => 'badge-enfermaria'
        ],
        'Inclusão' => [
            'cor' => '#20c997',
            'cor_texto' => '#ffffff',
            'icone' => 'fas fa-home',
            'classe' => 'badge-inclusao'
        ],
        'LGBT' => [
            'cor' => '#e83e8c',
            'cor_texto' => '#ffffff',
            'icone' => 'fas fa-rainbow',
            'classe' => 'badge-lgbt'
        ],
        'Triagem' => [
            'cor' => '#6c757d',
            ...
            'classe' => 'badge-triagem'
        ],
        'Alojamento externo' => [
            'cor' => '#343a40',
            'cor_texto' => '#ffffff',
            'icone' => 'fas fa-building',
            'classe' => 'badge-alojamento'
        ],
        'Adaptação' => [
            'cor' => '#6610f2',
            'cor_texto' => '#ffffff',
            'icone' => 'fas fa-universal-access',
            'classe' => 'badge-adaptacao'
        ]
    ];

    return $tipos[$tipo_residencia] ?? [
        'cor' => '#6c757d',
        'cor_texto' => '#ffffff',
...
if ($resultado === null) {
    $estrutura_query = "
        ...
        GROUP BY i.galeria, i.bloco, i.ala, i.res, i.tipo_residencia
    ...
    if ($tem_mudanca_recente === null) {
        try {
            $mudanca_query = "
                ...
                AND h.data_alteracao >= DATE_SUB(NOW(), INTERVAL 48 HOUR)
                AND EXISTS (
                    SELECT 1 FROM internos_historico_detalhado h2
                    WHERE h2.ipen = h.ipen
                    AND h2.data_alteracao = h.data_alteracao
...
function obterInfoGaleria($galeria)
{
    $info = [
        ...
        'G' => ['bg' => '#e83e8c', 'text' => '#fff', 'label' => 'Galeria G (Isolamento)'],
        'H' => ['bg' => '#6c757d', 'text' => '#fff', 'label' => 'Galeria H (Cozinha/DAL)'],
        'SA' => ['bg' => '#ffc107', 'text' => '#000', 'label' => 'Galeria SA'],
        'SB' => ['bg' => '#ffb300', 'text' => '#000', 'label' => 'Galeria SB'],
        'SC' => ['bg' => '#ff9a00', 'text' => '#fff', 'label' => 'Galeria SC'],
        'SD' => ['bg' => '#ff8c00', 'text' => '#fff', 'label' => 'Galeria SD'],
        'SE' => ['bg' => '#f8b500', 'text' => '#000', 'label' => 'Galeria SE'],
        'ST' => ['bg' => '#ff9800', 'text' => '#fff', 'label' => 'Galeria ST'],
        'T' => ['bg' => '#495057', 'text' => '#fff', 'label' => 'Galeria T (Triagem/Castigo)'],
    ];
...
// ==================== ENDPOINT PARA LAYOUT DE IMPRESSÃO ====================
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'get_layout_data') {

    // Headers para resposta JSON
    header('Content-Type: application/json; charset=utf-8');

    try {
        // Retornar dados completos para o layout
        echo json_encode([
            'success' => true,
            'data' => [
                'estrutura_com_blocos' => $estrutura_com_blocos,
                'stats' => $stats,
                'ultima_atualizacao' => $ultima_atualizacao,
                'info_galerias' => [
                    'A' => obterInfoGaleria('A'),
                    'B' => obterInfoGaleria('B'),
                    'C' => obterInfoGaleria('C'),
                    'D' => obterInfoGaleria('D'),
                    'E' => obterInfoGaleria('E'),
                    'F' => obterInfoGaleria('F'),
                    'G' => obterInfoGaleria('G'),
                    'H' => obterInfoGaleria('H'),
                    'T' => obterInfoGaleria('T'),
                    'SA' => obterInfoGaleria('SA'),
                    'SB' => obterInfoGaleria('SB'),
                    'SC' => obterInfoGaleria('SC'),
                    'SD' => obterInfoGaleria('SD'),
                    'SE' => obterInfoGaleria('SE'),
                    'ST' => obterInfoGaleria('ST')
                ]
            ]
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Erro ao obter dados do layout: ' . $e->getMessage()
        ], JSON_UNESCAPED_UNICODE);
    }
