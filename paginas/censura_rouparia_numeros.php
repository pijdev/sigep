<?php
require_once __DIR__ . '/../includes/censura_rouparia_logica.php';
?>

<script>
    window.currentPage = 'censura_rouparia_numeros.php';
    ...
    window.getRelatorioUrl = function(mode, sortBy) {
        ...
        let url = 'paginas/censura_rouparia_numeros.php?execute_print=1&mode=' + mode;
    ...
    document.addEventListener('click', function(e) {
        ...
        if (target && target.href.includes('censura_rouparia_numeros.php')) {
            e.preventDefault();
...
<link rel="stylesheet" href="assets/css/censura_rouparia_numeros.css">
...
<section class="content pt-3">
    <div class="container-fluid">
        ...
        <!-- FILTROS -->
        <?php
        $result = $pdo->query("SELECT DISTINCT situacao FROM internos WHERE situacao!='' ORDER BY 1");
        $situacoes = $result ? $result->fetchAll(PDO::FETCH_COLUMN) : [];
        ?>
        <div class="card card-dark no-print">
            ...
            <div class="card-body">
                <form action="paginas/censura_rouparia_numeros.php" method="GET" id="formFiltro" onsubmit="window.reloadContent(this.action + '?' + $(this).serialize()); return false;">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 d-flex align-items-end mb-3 gap-2">
                            ...
                            <button type="button" class="btn btn-outline-secondary btn-sm" onclick="window.reloadContent('paginas/censura_rouparia_numeros.php')">Limpar</button>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <div class="card shadow">
                ...
                <table class="table table-head-fixed table-hover table-sm align-middle" id="tabIn">
                    ...
                    <tbody>
                        <?php foreach ($internos as $i):
                            $isR = in_array($i['kit'], $duplicados_ids);
                        ...
                        ?>
                            <tr class="row-<?= $ct ?> <?= $isR ? 'row-repetido' : '' ?> <?= $social_class ?>" data-ipen="<?= $i['ipen'] ?>">
                                <td class="text-center fw-bold"><?= $i['ipen'] ?></td>
                                <td class="small font-weight-bold"><?= $nome_exib ?></td>
                                ...
                                <td class="text-center"><input type="number" class="input-edit-sm kr-v" data-old="<?= $kr ?>" value="<?= $kr ?>" <?= $i['regalia'] !== 'S' ? 'disabled' : '' ?> oninput="window.checkChanges(<?= $i['ipen'] ?>)"></td>
            ...
            <div class="card-footer clearfix">
                ...
                <ul class="pagination pagination-sm m-0 float-right">
                    <?php function getPagLink($p)
                    {
                        $q = $_GET;
                        $q['page'] = $p;
                        return "paginas/censura_rouparia_numeros.php?" . http_build_query($q);
                    } ?>
    ...
    <div class='p-3' style='max-height: calc(100vh - 120px); overflow-y: auto; color: #333;'>
        ...
        <div class='grid-kits mb-4'>
            ...
            foreach ($k_livres_tela as $v) {
                ...
                echo "<div class='kit-badge $extra_class' id='badge-kit-$v' title='$title'>$v</div>";
            }
            ?>
        </div>
            ...
            <table class="table table-sm table-striped small border">
                ...
                <tbody>
                    <?php foreach ($kits_quarentena_tela as $kq):
                        ...
                        $dias_restantes = (strtotime($kq['data_liberacao']) - time()) / (60 * 60 * 24);
                        $dias_txt = ceil($dias_restantes);
                    ?>
                        <tr>
                            <td class="text-center font-weight-bold text-orange" style="vertical-align: middle; font-size: 1rem;"><?= $kq['kit'] ?></td>
                            <td style="vertical-align: middle;">
                                <span class="font-weight-bold"><?= mb_strtoupper($nome_exib) ?></span>
                            ...
                            <td class="text-center font-weight-bold" style="vertical-align: middle; color: #28a745;">
                                <?= $data_lib ?>
                                <div style="font-size: 0.65rem; color: #666; font-weight: normal;">(<?= $dias_txt ?> dias)</div>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
...
<div class='offcanvas-custom' id='off_sem' style='visibility: hidden; position: fixed; top: 0; right: 0; width: 500px; height: 100%; z-index: 1050; background: #fff; border-left: 1px solid #ddd; transition: transform .3s ease-in-out; transform: translateX(100%);'>
    <div class='p-3 border-bottom d-flex justify-content-between align-items-center bg-light'>
        ...
        <div><button class='btn btn-sm btn-default mr-1' onclick="window.open('paginas/censura_rouparia_numeros.php?execute_print=1&mode=sem_kit', '_blank')" <?= ($_SESSION['user_nome'] == 'Rouparia' ? 'disabled' : '') ?>><i class='fas fa-print'></i> Imprimir</button><button class='btn btn-sm btn-secondary' onclick="window.exibOff('off_sem', false)" title="Fechar"><i class='fas fa-chevron-right'></i></button></div>
    </div>
    <div class='p-3' style='max-height: calc(100vh - 70px); overflow-y: auto; color: #333;'>
        <table class='table table-sm table-striped small'>
            ...
            <tbody><?php foreach ($sem_k_tela as $sk): $n = !empty($sk['nome_social']) ? "<b>{$sk['nome_social']}</b><br><small>({$sk['nome']})</small>" : $sk['nome']; ?><tr>
                        <td><?= $sk['ipen'] ?></td>
                        <td><?= $n ?></td>
...
<div class='offcanvas-custom' id='off_rep' style='visibility: hidden; position: fixed; top: 0; right: 0; width: 500px; height: 100%; z-index: 1050; background: #fff; border-left: 1px solid #ddd; transition: transform .3s ease-in-out; transform: translateX(100%);'>
    <div class='p-3 border-bottom d-flex justify-content-between align-items-center bg-light'>
        ...
        <div><button class='btn btn-sm btn-default mr-1' onclick="window.open('paginas/censura_rouparia_numeros.php?execute_print=1&mode=conflitos', '_blank')" <?= ($_SESSION['user_nome'] == 'Rouparia' ? 'disabled' : '') ?>><i class='fas fa-print'></i> Imprimir</button><button class='btn btn-sm btn-secondary' onclick="window.exibOff('off_rep', false)" title="Fechar"><i class='fas fa-chevron-right'></i></button></div>
    </div>
    <div class='p-3' style='max-height: calc(100vh - 70px); overflow-y: auto; color: #333;'>
        <table class='table table-sm table-striped small'>
            <thead>
            ...
            <tbody><?php foreach ($repet_tela as $r): $n = !empty($r['nome_social']) ? "<b>{$r['nome_social']}</b>" : $r['nome']; ?><tr>
...
<div class='offcanvas-custom' id='off_regalias_fechado' style='visibility: hidden; position: fixed; top: 0; right: 0; width: 600px; height: 100%; z-index: 1050; background: #fff; border-left: 1px solid #ddd; transition: transform .3s ease-in-out; transform: translateX(100%);'>
    <div class='p-3 border-bottom bg-light'>
        <div class='d-flex justify-content-between align-items-center mb-2'>
            ...
            <div>
                <button class='btn btn-sm btn-default mr-1' onclick="window.open('paginas/censura_rouparia_numeros.php?execute_print=1&mode=regalias_fechado', '_blank')" <?= ($_SESSION['user_nome'] == 'Rouparia' ? 'disabled' : '') ?>><i class='fas fa-print'></i> Imprimir</button>
...
<div class='offcanvas-custom' id='off_regalias_semiaberto' style='visibility: hidden; position: fixed; top: 0; right: 0; width: 600px; height: 100%; z-index: 1050; background: #fff; border-left: 1px solid #ddd; transition: transform .3s ease-in-out; transform: translateX(100%);'>
    <div class='p-3 border-bottom bg-light'>
        <div class='d-flex justify-content-between align-items-center mb-2'>
            ...
            <div>
                <button class='btn btn-sm btn-default mr-1' onclick="window.open('paginas/censura_rouparia_numeros.php?execute_print=1&mode=regalias_semiaberto', '_blank')" <?= ($_SESSION['user_nome'] == 'Rouparia' ? 'disabled' : '') ?>><i class='fas fa-print'></i> Imprimir</button>
...
<script src="assets/js/censura_rouparia_numeros.js?v=<?= time() ?>"></script>
